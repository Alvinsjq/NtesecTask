 ...
 */
public class DIV {
    public StringBuffer DIV = new StringBuffer();
    public int k1 = 1; //x1的系数
    public int k2 = 1; //x2的系数
    public int c1 = 0;  //输出偏置参数
    public int c2 = 0;  //输出偏置参数

    public StringBuffer _DIV_(String x1,String x2){
        DIV.append(String.valueOf("("+getK1())).append("*").append(x1).append(" + ").append(getC1())
                .append(") / (").append(getK2()).append("*").append(x2).append(" + ").append(getC2())
                .append(");");
        return DIV;
    }

    public StringBuffer getDIV() {
        return DIV;
    }
